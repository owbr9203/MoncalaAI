...
class Mancala:
    ...
    def valid_moves(self, board, current_player):
        ...
        if current_player == 1: #For P1
            for pit in range(self.p1_mancala_index): #For every P1 pit other than their mancala...
                if board[pit] > 0: #If that pit has stones in it
    ...
    def play(self, pit, board, current_player):
        ...
        while stones > 0:
            current_pit = (current_pit + 1) % len(board)  # Moves to the next pit
            if (current_player == 1 and current_pit == self.p2_mancala_index) or (
                    current_player == 2 and current_pit == self.p1_mancala_index):
                current_pit = (current_pit + 1) % len(board)  # Skips the opponent's mancala
            board[current_pit] += 1  # Drops a stone in the current pit
            stones -= 1  # Decrements the number of stones


        # checks for if you end on pit
        if current_player == 1 and current_pit in range(self.p1_pits_index[0], self.p1_pits_index[1] + 1) and board[current_pit] == 1:
            opposite_pit = self.p2_mancala_index - current_pit - 1  # Calculates the opposite pit
            
            board[current_pit] = 0  # Empties the current pit

            board[self.p1_mancala_index] += board[opposite_pit] + 1# Captures the stones in the opposite pit
            board[opposite_pit] = 0  # Empties the opposite pit
                

        elif current_player == 2 and current_pit in range(self.p2_pits_index[0], self.p2_pits_index[1] + 1) and board[current_pit] == 1:
            opposite_pit = self.p2_mancala_index - current_pit - 1  # Calculates the opposite pit
            
            board[current_pit] = 0  # Empties the current pit
            board[self.p2_mancala_index] += board[opposite_pit] + 1 # Captures the stones in the opposite pit
            board[opposite_pit] = 0  # Empties the opposite pit


        self.moves.append((current_player, pit + 1))  # Records the move
        current_player = 2 if current_player == 1 else 1  # Switches the current player
        
        if self.winning_eval(board, current_player) != 0:
            # print("GAME OVER")
            return self.winning_eval(board, current_player) # Exits if the game is over
