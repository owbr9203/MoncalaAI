...
def play_game_alphabeta(player1: player, player2: player, game:m):
    playing = True
    curr_player = 1
    turns = 0
    
    while(playing):
        status = game.play(game.alphabeta_search())
        # print(status, "stones: ", find_total_stones(game))
        # game.display_board()
        turns+=1
        if status < 0 :
...
def main():
    ...
    """ for i in range(100):
        ...
        game = m.Mancala()
        total_turns += play_rand_game(player1, player2, game)
    ...
    play_game_alphabeta(player1,player2,game)
    print(game.display_board())
